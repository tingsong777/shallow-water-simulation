...
% 確認變數存在
if ~exist('x', 'var') || ~exist('eta_time_series', 'var') || ~exist('t_series', 'var')
    error('檔案中沒有變數 x、eta_time_series 或 t_series');
end
...
% 繪圖範圍設定
y_min = min(eta_time_series(:));
y_max = max(eta_time_series(:));
...
for i = 1:length(special_times)
    eta_snapshot = eta_time_series(special_indices(i), 3:end-2);  % 去掉邊界資料
...
figure;
h_plot = plot(x, eta_time_series(1, 3:end-2), 'b-', 'LineWidth', 1.5);
xlabel('x (meters)');
ylabel('\eta');
ylim([y_min, y_max]);
...
% 動畫播放
for k = 1:size(eta_time_series, 1)
    eta_snapshot = eta_time_series(k, 3:end-2);
