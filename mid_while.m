...
while t_now < t_max
    ...
    t_now = t_now + dt;
    fprintf('Time progress: %.4f / %.2f sec\n', t_now, t_max);
    t_series = [t_series, t_now];
    ...
    U_2 = zeros(1, nx+4);
        ...
        %============
    for j = 3:nx+2
        eta_1(j) = eta(j) - (dt/12/dx) * (-U(j+2)*h(j+2) + 8*U(j+1)*h(j+1) - ...
            8*U(j-1)*h(j-1) + U(j-2)*h(j-2));
        U_1(j) = U(j) - (dt/12/dx * g) * (-eta(j+2) + 8*eta(j+1) - ...
            8*eta(j-1) + eta(j-2));
    end
        % apply B.C.
        eta_1(1) = eta_1(5);
        eta_1(2) = eta_1(4);
        % eta_1(end-1) = eta_1(end-3);
        ...
        %============= 
    for j = 3:nx+2
        eta_2(j) = (3/4) * eta(j) + (1/4) * (eta_1(j) - (dt/12/dx) ...
            * (-U_1(j+2)*h(j+2) + 8*U_1(j+1)*h(j+1) - 8*U_1(j-1)*h(j-1) + U_1(j-2)*h(j-2)));
        U_2(j) = (3/4) * U(j) + (1/4) * (U_1(j) - (dt/12/dx * g) * ...
    ...
    U_new = U;
    for j = 3:nx+2
        eta_new(j) = (1/3) * eta(j) + (2/3) * (eta_2(j) - (dt/12/dx) ...
            * (-U_2(j+2)*h(j+2) + 8*U_2(j+1)*h(j+1) - 8*U_2(j-1)*h(j-1) + U_2(j-2)*h(j-2)));
        U_new(j) = (1/3) * U(j) + (2/3) * (U_2(j) - (dt/12/dx * g) ...
            * (-eta_2(j+2) + 8*eta_2(j+1) - 8*eta_2(j-1) + eta_2(j-2)));
    end
        % apply B.C.
        eta_new(1) = eta_new(5);
        eta_new(2) = eta_new(4);
        % eta_new(end-1) = eta_new(end-3);
        % eta_new(end) = eta_new(end-4);
        U_new(1) = -U_new(5);
        U_new(2) = -U_new(4);
        % U_new(end-1) = -U_new(end-3);
        % U_new(end) = -U_new(end-4);
    % **Update variables**
    eta = eta_new;
    U = U_new;

    % **Store in time series**
    eta_time_series = [eta_time_series; eta];
    % U_time_series = [U_time_series; U];
